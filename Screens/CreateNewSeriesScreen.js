...
export default function CreateNewSeries() {
  ...
  const [heroUri, setHeroUri] = React.useState(null);
  ...
  }, [logoImg, thumbnailImg, heroImg, name, slug, description]);
  ...
  const saveNewSeries = () => {
    ...
    if (
      ...
      setIsloading(true);
      dispatch(postSeries({ token: token, data: formData })).then((res) => {
        if (res.payload.data.body) {
          dispatch(getAllSeries({ token: token }));
          setIsloading(false);
          dispatch(
            getSeriesById({ token: token, id: res.payload.data.body.id })
          ).then((res) => {
            if (res.payload.data.body) {
